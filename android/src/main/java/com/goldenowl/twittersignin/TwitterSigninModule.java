...
public class TwitterSigninModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    public void logIn(final Promise promise) {
        ...
        twitterAuthClient.authorize(getCurrentActivity(), new com.twitter.sdk.android.core.Callback<TwitterSession>() {
            ...
            public void success(Result<TwitterSession> result) {
                ...
                try{
                    prefs.edit().putString("userId",Long.toString(session.getUserId())).commit();
                }
                catch(Exception e){
                    map.putString("pref", e.toString());
                ...
                //     }
                // });//commened by jose and below code added by jose
                TwitterCore.getInstance().getApiClient(session).getAccountService().verifyCredentials(false,true,true).enqueue(new Callback<User>() {
                    ...
                    public void success(Result<User> userResult) {
                        try {
                            ...
                            map.putString("name", userResult.data.screenName);
                            map.putString("avatar", userResult.data.profileImageUrlHttps);
                            promise.resolve(map);
                        } catch (Exception e) {
                            map.putString("email", "COULD_NOT_FETCH");
                            promise.reject("COULD_NOT_FETCH", map.toString());
    ...
    //added to get current session jose2007kj
    @ReactMethod
    public void getUserId(final Promise promise) {
        final WritableMap map = Arguments.createMap();
        try{
            map.putString("userId", prefs.getString("userId",""));
            promise.resolve(map);
        }catch(Exception e){
            
            map.putString("userId", e.toString());
            promise.reject("COULD_NOT_FETCH", map.toString());
        }
    }
    // @ReactMethod
    // public void clearUserId(final Promise promise) {
    //     final WritableMap map = Arguments.createMap();
    //     try{
